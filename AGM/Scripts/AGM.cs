...
ShieldComponent=2000";
        ...
        private void RunWarningLights()
        {
            for (int i=0; i<_blocks.Count; i++)
            {
                var b=_blocks[i]; if (b.CustomName.IndexOf(LIGHT_TAG,SC)<0) continue;
                var light=b as IMyLightingBlock; if (light==null) continue;
                string name=b.CustomName.ToUpperInvariant();
                int state=ALERT_OK;
                if      (name.IndexOf("REACTOR CHARGING",SC)>=0||name.IndexOf("CHARGING",SC)>=0) state=(_reactorsForcedOn&&_amberWhileCharging)?ALERT_WARNING:ALERT_OK;
                else if (name.IndexOf("POWER OK",SC)  >=0) state=(_alertBattery==ALERT_OK&&!_powerSafetyHold)?ALERT_OK:ALERT_WARNING;
                else if (name.IndexOf("BATTERY",SC)   >=0) state=_alertBattery;
                else if (name.IndexOf("CARGO",SC)     >=0) state=_alertCargo;
                else if (name.IndexOf("HYDROGEN",SC)  >=0) state=_alertHydrogen;
                else if (name.IndexOf("OXYGEN",SC)    >=0) state=_alertOxygen;
                else if (name.IndexOf("URANIUM",SC)   >=0) state=_alertUranium;
                else if (name.IndexOf("PRODUCTION",SC)>=0) state=_alertProduction;
        ...
        private void BuildProductionLists()
        {
            ...
            for (int i=0; i<_blocks.Count; i++)
            {
                ...
                var asm=b as IMyAssembler; if (asm!=null&&asm.IsFunctional&&!HasToken(asm.CustomName,_manualTag)&&MatchSpec(asm,_prodAssemblers,true)){_assemblers.Add(asm);continue;}
        ...
        private int QueueCompQuotas()
        {
            ...
            foreach (var quota in _compQuotas)
            {
                ...
                asm.AddQueueItem(bp,(MyFixedPoint)amount); queued++; _lastQueuedItem=amount.ToString("0")+" "+quota.Key;
            }
            return queued;
        }
        ...
        { _queue.Clear(); a.GetQueue(_queue); if(_queue.Count==0)return a.IsProducing?"WORKING":"IDLE"; return Trim(CompFromBP(_queue[0].BlueprintId),24); }
        ...
        private void DrawPbScreen()
        {
            ...
            using (var fr=surf.DrawFrame())
            {
                ...
                DrawBusAnim(fr,new RectangleF(panel.X,panel.Y+70f,panel.Width,130f));
                ...
                Row(fr,panel,y,"Alerts",     _alertsEnabled?"ONLINE":"OFF",     _alertsEnabled?AlertColor(_alertOverall):COL_DIM);y+=30f;
        ...
        private void DrawCoreDash(IMyTextSurface s)
        {
            ...
            using (var fr=s.DrawFrame())
            {
                ...
                Row(fr,panel,y,"Alerts",_alertsEnabled?"ON":"OFF",    _alertsEnabled?COL_OK:COL_DIM);y+=32f;
                Row(fr,panel,y,"Alert status",AlertLabel(_alertOverall),AlertColor(_alertOverall));y+=32f;
        ...
        private void DrawProductionDash(IMyTextSurface s, int page)
        {
            ...
            using (var fr=s.DrawFrame())
            {
                ...
                if (page==2&&_prodV2&&_prodShowDetails)
                {
                    ...
                    for(int i=0;i<_assemblers.Count&&used<rows;i++,used++,y+=32f) Row(fr,panel,y,Trim(MachineName(_assemblers[i].CustomName),24),AsmJob(_assemblers[i]),_assemblers[i].IsProducing?COL_OK:COL_DIM);
