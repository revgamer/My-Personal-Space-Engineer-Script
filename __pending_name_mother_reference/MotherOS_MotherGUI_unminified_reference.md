...
- Controls blocks directly: doors, lights, pistons, rotors, hinges, batteries, tanks, connectors, sorters, thrusters, wheels, timers, programmable blocks, screens, etc.
...
| `dock <Grid Name> [--local=...] [--remote=...]` | Dock with a grid using connectors. |
...
| Command | Purpose |
|---|---|
| `connector/lock <Connector|Group>` | Lock connector. |
| `connector/unlock <Connector|Group>` | Unlock connector. |
| `connector/toggle <Connector|Group>` | Toggle connector lock. |

Hooks include lock/unlock/ready style transitions.
...
- Landing gear/connectors: `onLock`, `onUnlock`, ready-to-lock style events
